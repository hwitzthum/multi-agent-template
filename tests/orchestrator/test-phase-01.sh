...
expect_failure "Bash-Guard blockiert Push mit JSON-Tabulator" sh -c "printf '%s' '{\"command\":\"git\\\\tpush origin main\"}' | '$project_dir/scripts/bash-guard.sh'"
expect_failure "Bash-Guard blockiert rekursives Löschen mit JSON-Tabulator" sh -c "printf '%s' '{\"command\":\"rm\\\\t-rf build\"}' | '$project_dir/scripts/bash-guard.sh'"
expect_failure "Bash-Guard blockiert --no-verify" sh -c "printf '%s' '{\"command\":\"git commit --no-verify -m x\"}' | '$project_dir/scripts/bash-guard.sh'"
expect_success "Bash-Guard lässt normalen Commit zu" sh -c "printf '%s' '{\"command\":\"git commit -m x\"}' | '$project_dir/scripts/bash-guard.sh'"
...
[ "$(sed -n '1p' "$lock/pid")" = "$$" ] && ok || bad "uebernommene Sperre traegt die eigene PID"
